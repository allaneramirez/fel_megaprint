...
class FelMegaprint(models.Model):
    ...
    #motivo fel deberia de usarse aca tambien como en ASF
    motivo_anula_fel = fields.Char('Motivo de anulación',
                                   ...
                                   readonly=False, states={'draft': [('readonly', False)]})
    # aqui no se usa pero en contingencia si y en account .py
    ...
    # SOLO SI SE INGRESA ESCENARIO POR LA FACTURA
    ...
    # Campos que no se ingresan numero_acceso_fel, lugar_expedicion_fel, otra_referencia_fel, no se usa frase_exento_fel
    # campos para guardar datos no se usan aca pero si en otro lado
    # documento_xml_fel, documento_xml_fel_name, resultado_xml_fel, resultado_xml_fel_name,
    # en la fuclnion requiere certificaion certificador_fel
...
#         # USA MOTIVO ANULA FEL EN LUGAR DE MOTIVO_FEL DE ASFALGUA
...
#
#         tipo_documento_fel = factura.journal_id.tipo_documento_fel
#         ##### tipo_interno_factura no afecta en nada ###
#         if tipo_documento_fel in ['FACT', 'FACM'] and factura.move_type == 'out_refund':
#             tipo_documento_fel = 'NCRE'
#
#         moneda = "GTQ"
...
#             moneda = "USD"
#
#         fecha_hora_creacion = (factura.create_date) or fields.Datetime.now()
#         fecha_hora = fecha_hora_creacion.strftime('%Y-%m-%dT%H:%M:%S') + '-06:00'
#
#         ### DEJAR FECHA DE ASFALGUA
#         # fecha = factura.invoice_date.strftime('%Y-%m-%d') if factura.invoice_date else fields.Date.context_today(
#         #     self).strftime('%Y-%m-%d')
#         # hora = "00:00:00-06:00"
#         # fecha_hora = fecha + 'T' + hora
#
#         ### OMITE NumeroAcceso=numero_acceso factura.numero_acceso_fel
...
#
#         ## OMITE if factura.company_id.tipo_personeria_fel: TIPO TESORERIA
...
#         ####################### agrega tipo frase, codigo_escenario ####################################################
#         ############# DEJAR COMO EN ASFALGUA
...
#             ###### en ASF tenemos and not enviar_lineas_en_cero_fel
...
#             # / len(linea.tax_ids) > 0:
#             #AQUI ASF ITERA SOBRE LOS IMPUESTOS DETERMINADOS
#
...
#
#         # if cantidad_impuestos > 0: no busca frase ##################### OJO
#         if tipo_documento_fel not in ['NABN']:
#             # EN ASF SE AGREGA ANTES EL TOTALIMPUESTOS
...
#             #### Aqui itera sobre impuestos
#
#         GranTotal = etree.SubElement(Totales, DTE_NS + "GranTotal")
#         GranTotal.text = '{:.2f}'.format(factura.currency_id.round(gran_total))
#
#         # En todos estos casos, es necesario enviar complementos (ESTO LO PODEMOS DEJAR EXACTAMENTE IGUAL) no agrega frases frase_exento_fel
...
#                     # no ingresa motivoajsute conb el campo motivo_nota, motivo_fel en ASFEN AMBOSD IF
...
#                 # no ingresa lugar_expedicion_fel
...
#                 # no se ingresa otra_referencia_fel
